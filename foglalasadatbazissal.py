...
# Azonos mappában tárolja a 'foglalasok.txt' fájlt a scripttel
current_dir = os.path.dirname(os.path.abspath(__file__))
file_path = os.path.join(current_dir, 'foglalasok.txt')
...
class Szalloda:
    ...
    def foglalasok_mentese(self):
        with open(file_path, 'w', newline='') as file:
            writer = csv.writer(file)
            for f in self.foglalasok:
                writer.writerow([f.szobaszam, f.datum])

    def foglalasok_betoltese(self):
        try:
            with open(file_path, 'r', newline='') as file:
                reader = csv.reader(file)
                for row in reader:
                    if row:
                        szobaszam, datum = int(row[0]), date.fromisoformat(row[1])
                        self.foglalasok.append(Foglalas(szobaszam, datum))
        except FileNotFoundError:
            pass  # Fájl nem létezik, első használat
...
def felhasznaloi_interfesz():
    ...
    szalloda.foglalasok_betoltese()  # Betöltjük a korábbi foglalásokat
